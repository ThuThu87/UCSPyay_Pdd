<?php
    ...
    $start_from = ($page-1) * $limit; 
                    if($role==1)
                    {
                      $sql = "SELECT *  FROM allimages ORDER BY id ASC LIMIT $start_from, $limit";
                    }
                    else 
                    {
                      $sql = "SELECT *  FROM allimages where pgdName='$pgdName' ORDER BY id ASC LIMIT $start_from, $limit";
                      } 
?>
<!DOCTYPE html>
  <html lang="en">
    ...
    <div class="bg-light d-flex flex-row align-items-center mt-3 mb-5">
      <div class="container">
        <div class="row justify-content-center table-responsive">
          <div class="col-md-12">
            <div class="col d-flex p-2 justify-content-center" style="color:dodgerblue;font-weight:bolder;font-size:large;">ပွဲတော်နှင့်ဆိုင်သောဓါတ်ပုံမှတ်တမ်းများ</div>
            <font size="2rem">
            <?php
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>
                                    ...
                                    <td>{$row['fName']}</td>
                                    <td>{$row['monthName']}</td>                                                           
                                    <td><img src='{$row['file_name']}'  width='100' height='100'></td>
                                    <td>
                                    <a href='deleteFestivalPhoto.php?id={$row['id']}&pid={$p_id}' onclick='return confirm('Are you sure you want to delete this item?')'><i class='fas fa-trash fa-lg'></i></a>
                                    </td>
                                </tr>";
                                $i++;
                        }
...
?>
    ...
    <?php 
     if($role==1)
     {
      $result_db = mysqli_query($conn,"SELECT COUNT(id) FROM allimages");
     ...
     else
     {
      ...
      for ($i=1; $i<=$total_pages; $i++) {
                    $pagLink .= "<li class='page-item' align='center'><a class='page-link' align='center' href='viewFestivalPhoto?page=".$i."'>".$i."</a></li>";	
      }
      echo $pagLink . "</ul>";
      $conn->close();
      ?>
