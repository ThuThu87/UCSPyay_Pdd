        ...
        <div class="container-fluid">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb my-0 ms-2">
              ...
              <li class="breadcrumb-item active"><span>UpdateGeneral</span></li>
            </ol>
          </nav>
        </div>
          <!-- <div class="header-divider"></div> -->
        ...
        <div class="row justify-content-center">
          <div class="col-md-10">
            <div class="card mb-4 mx-4">
              <div class="card-title"><h3><b>အထွေထွေအကြောင်းအရာများပြင်ရန်</b></h3></div>
           ...
           <form action="updateGeneral" method="post" enctype="multipart/form-data">
                  ...
                  $g_id=$_GET['gid'];
                  $p_id=$_GET['pid']; 
                  $result = mysqli_query($conn,"SELECT * FROM general where p_id='$p_id' and g_id='$g_id'");
                   if($row=mysqli_fetch_array($result)){
                  ?>                         
        ...
        if(in_array($fileType, $allowTypes)){ 
          ...
          // $imgContent1 = addslashes(file_get_contents($image1));
            ...
            if((empty($image_full_path)) || (empty($image_full_path1)) || ( empty($detail))){
              $result = mysqli_query($conn,"SELECT * FROM national where n_id='$subCatName' where p_id='.$row[1].'");
              if($row=mysqli_fetch_array($result)){
                if($image_full_path=='upload/general/')
                   $image_full_path=implode($row['icon']);
                if($image_full_path1=='upload/general/')
           ...
           $qry="Update general set icon='$image_full_path', photo='$image_full_path1',detail_info='$detail' where g_id='$subCatName'"; 
