<?php
    ...
    if($role==1)
                    {
                      $sql = "SELECT *  FROM national ORDER BY n_id ASC LIMIT $start_from, $limit";
                    }
                    else 
                    {
                      $sql = "SELECT *  FROM national where pgdName='$pgdName' ORDER BY n_id ASC LIMIT $start_from, $limit";
                      } 
?>
<!DOCTYPE html>
  <html lang="en">
        ...
        <div class="container-fluid">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb my-0 ms-2">
              ...
              <li class="breadcrumb-item active"><span>View National</span></li>
            </ol>
          </nav>
          <div align="right">
    ...
    <div class="bg-light d-flex flex-row align-items-center mt-3 mb-5">
      <div class="container">
        <div class="row justify-content-center table-responsive">
          <div class="col-md-12">
            <div class="col d-flex p-2 justify-content-center" style="color:dodgerblue;font-weight:bolder;font-size:large;">ခေတ်အဆက်ဆက်ပြုပြင်ထိန်းသိမ်းမှု့များ</div>
            <font size="2rem">
            <?php
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>
                                    ...
                                    <td >$substr_detail</td>
                                    <td>
                                        <a href='updateNational.php?nid={$row['n_id']}&pid={$row['p_id']}'><i class='fas fa-edit fa-lg'></i></a>                                        
                                    </td>
                                    <td>
                                    <a href='deleteNational.php?nid={$row['n_id']}&pid={$row['p_id']}' onclick='return confirm('Are you sure you want to delete this item?')'><i class='fas fa-trash fa-lg'></i></a>
                                    </td>
                                </tr>";
                                $i++;
                        }
...
?>
    ...
    if($role==1)
     {
      $result_db = mysqli_query($conn,"SELECT COUNT(n_id) FROM national");

     }
     else
     {
      $result_db = mysqli_query($conn,"SELECT COUNT(n_id) FROM national where pgdName='$pgdName'"); 
     }
      ...
      for ($i=1; $i<=$total_pages; $i++) {
                    $pagLink .= "<li class='page-item' align='center'><a class='page-link' align='center' href='viewNational?page=".$i."'>".$i."</a></li>";	
      }
      echo $pagLink . "</ul>";
      $conn->close();
      ?>
