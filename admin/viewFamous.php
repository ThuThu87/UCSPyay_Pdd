<?php
    ...
    $start_from = ($page-1) * $limit; 
                    if($role==1)
                    {
                      $sql = "SELECT *  FROM famous_places ORDER BY f_id ASC LIMIT $start_from, $limit";
                    }
                    else 
                    {
                      $sql = "SELECT *  FROM famous_places where pgdName='$pgdName' ORDER BY f_id ASC LIMIT $start_from, $limit";
                      } 
?>
        ...
        <div class="container-fluid">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb my-0 ms-2">
              ...
              <li class="breadcrumb-item active"><span>View Famous_Place</span></li>
            </ol>
          </nav>
        </div>
          <!-- <div class="header-divider"></div> -->
    ...
    <div class="bg-light d-flex flex-row align-items-center mt-3 mb-5">
      <div class="container">
        <div class="row justify-content-center table-responsive">
          <div class="col-md-12">
            <div class="col p-2 justify-content-center" style="color:dodgerblue;font-weight:bolder;font-size:large;">ဖူးမြော်လေ့လာနိုင်သောနေရာများ</div>
            <font size="2rem">
            <?php
                        ...
                        while ($row = $result->fetch_assoc()) {
                          $substr_detail=mb_substr($row['detail_info'],0,200)."......."; 
                            echo "<tr>
                                    ...
                                    <td >$substr_detail</td>
                                    <td>
                                        <a href='updateFamous.php?fid={$row['f_id']}&pid={$row['p_id']}'><i class='fas fa-edit fa-lg'></i></a>                                        
                                    </td>
                                    <td>
                                    <a href='deleteFamous.php?fid={$row['f_id']}&pid={$row['p_id']}' onclick='return confirm('Are you sure you want to delete this item?')'><i class='fas fa-trash fa-lg'></i></a>
                                    </td>
                                </tr>";
                                $i++;
                        }
...
?>
    ...
    <?php 
     if($role==1)
     {
      $result_db = mysqli_query($conn,"SELECT COUNT(f_id) FROM famous_places");

     }
     else
     {
      $result_db = mysqli_query($conn,"SELECT COUNT(f_id) FROM famous_places where pgdName='$pgdName'"); 
     }
      ...
      for ($i=1; $i<=$total_pages; $i++) {
                    $pagLink .= "<li class='page-item' align='center'><a class='page-link' align='center' href='viewFamous?page=".$i."'>".$i."</a></li>";	
      }
      echo $pagLink . "</ul>";
      $conn->close();
      ?>
