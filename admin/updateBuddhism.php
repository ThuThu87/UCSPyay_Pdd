        ...
        <div class="container-fluid">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb my-0 ms-2">
              ...
              <li class="breadcrumb-item active"><span>Update Buddhism</span></li>
            </ol>
          </nav>
          <div align="right">
        ...
        <div class="row justify-content-center">
          <div class="col-md-10">
            <div class="card mb-4 mx-4">
              <div class="card-title"><h3><b>သာသနာထွန်းကားမှုများပြင်ရန်</b></h3></div>
              <div class="card-body p-7">
           <form action="updateBuddhism" method="post" enctype="multipart/form-data">
          
                      
                  <?php
                  $b_id=$_GET['bid'];
                  $p_id=$_GET['pid']; 
                  $result = mysqli_query($conn,"SELECT * FROM buddhism where p_id='$p_id' and b_id='$b_id'");
                   if($row=mysqli_fetch_array($result)){
                  ?>                         
   ...
   // Allow certain file formats 
    ...
    if(in_array($fileType, $allowTypes)){ 
      ...
      // $imgContent1 = addslashes(file_get_contents($image1));
        ...
        echo $image_full_path1;
       
        if(($image_full_path=='upload/buddhism/') ||($image_full_path1=='upload/buddhism/') || ( empty($detail))){
           $result = mysqli_query($conn,"SELECT * FROM buddhism where  b_id='$subCatName' and p_id='$pgdId' ");
          if($result->num_rows>0){        
           if($row=mysqli_fetch_array($result)){
               if($image_full_path=='upload/buddhism/')
                   $image_full_path=$row['icon'];
               if($image_full_path1=='upload/buddhism/')
       ...
       $qry="Update buddhism set icon='$image_full_path', photo='$image_full_path1',detail_info='$detail' where b_id='$subCatName' and p_id='$pgdId'"; 
      ...
      { 
       ...
       window.location.href = "viewBuddhism";
       </script>'; 
      }  
       else
         {
