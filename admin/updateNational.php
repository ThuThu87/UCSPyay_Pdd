        ...
        <div class="container-fluid">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb my-0 ms-2">
              ...
              <li class="breadcrumb-item active"><span>UpdateNational</span></li>
            </ol>
          </nav>
          <div align="right">
        ...
        <div class="row justify-content-center">
          <div class="col-md-10">
            <div class="card mb-4 mx-4">
              <div class="card-title"><h3><b>ခေတ်အဆက်ဆက်ပြုပြင်မွမ်းမံထိန်းသိမ်းမှုများပြင်ရန်</b></h3></div>
              <div class="card-body p-7">
           <form action="updateNational" method="post" enctype="multipart/form-data">
          
                      
                  <?php
                  $n_id=$_GET['nid'];
                  $p_id=$_GET['pid']; 
                  $result = mysqli_query($conn,"SELECT * FROM national where p_id='$p_id' and n_id='$n_id'");
                   if($row=mysqli_fetch_array($result)){
                  ?>                         
                ...
                <div class="input-group mb-3 mt-4" style="font-size:.5rem;">
                  ...
                  <?php                  
                  $result = mysqli_query($conn,"SELECT * FROM national");
       ...
       // Allow certain file formats 
        ...
        if(in_array($fileType, $allowTypes)){ 
          ...
          // $imgContent1 = addslashes(file_get_contents($image1));
            ...
            if(($image_full_path=='upload/national/') ||($image_full_path1=='upload/national/') || ( empty($detail))){
              $result = mysqli_query($conn,"SELECT * FROM national where n_id='$subCatName' and p_id='$pgdId'");
             
                if($result->num_rows>0){        
                  if($row=mysqli_fetch_array($result)){
                      if($image_full_path=='upload/national/')
                          $image_full_path=$row['icon'];
                      if($image_full_path1=='upload/national/')
           ...
           $qry="Update national set icon='$image_full_path', photo='$image_full_path1',detail_info='$detail' where n_id='$subCatName' and pgdName='$pgdName'"; 
           $res=mysqli_query($conn,$qry);            
           if($res)     
           { 
            echo '<script>
            window.alert("Your Updated Data successfully");
            window.location.href = "viewNational";
            </script>'; 
           }  
            else
              {
