        ...
        <div class="container-fluid">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb my-0 ms-2">
              ...
              <li class="breadcrumb-item active"><span>Update Famous_Place</span></li>
            </ol>
          </nav>
          <div align="right">
        ...
        <div class="row justify-content-center">
          <div class="col-md-10">
            <div class="card mb-4 mx-4">
              <div class="card-title"><h3><b>ဖူးမြော်နိုင်သောနေရာများပြင်ရန်</b></h3></div>
              <div class="card-body p-7">
           <form action="updateFamous" method="post" enctype="multipart/form-data">
          
                      
                  <?php
                  $f_id=$_GET['fid'];
                  $p_id=$_GET['pid']; 
                  $result = mysqli_query($conn,"SELECT * FROM famous_places where p_id='$p_id' and f_id='$f_id'");
                   if($row=mysqli_fetch_array($result)){
                  ?>                         
       ...
       // Allow certain file formats 
        ...
        if(in_array($fileType, $allowTypes)){ 
          ...
          // $imgContent1 = addslashes(file_get_contents($image1));
            ...
            if(($image_full_path=='upload/famous_places/') ||($image_full_path1=='upload/famous_places/') || ( empty($detail))){
              $result = mysqli_query($conn,"SELECT * FROM famous_places where f_id='$subCatName' and p_id='$pgdId'");
              if($result->num_rows>0){ 
                if($row=mysqli_fetch_array($result)){
                  if($image_full_path=='upload/famous_places/')
                      $image_full_path=$row['icon'];
                  if($image_full_path1=='upload/famous_places/')
           ...
           $qry="Update famous_places set icon='$image_full_path', photo='$image_full_path1',detail_info='$detail' where f_id='$subCatName' and p_id='$pgdId'"; 
           $res=mysqli_query($conn,$qry);            
           if($res)     
           { 
            echo '<script>
            window.alert("Your Updated Data successfully");
            window.location.href = "viewFamous";
            </script>'; 
           }  
            else
              {
