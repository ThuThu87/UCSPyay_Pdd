        ...
        <div class="row justify-content-center">
          <div class="col-md-8">
            ...
            <div class="card-title" >
              <h3><b>မှတ်တမ်းဗီဒီယိုများထည့်ရန်<b></h3>
              </div>
              <div class="card-body p-7">               
           <form action="insertFestivalVideo" method="post" enctype="multipart/form-data">
  ...
  else
  {
    ...
    if($pgdName=='ဘုရားအမည်'){
      echo '<script>
        ...
        window.location.href = "insertFestivalVideo";
        </script>';
    }
    ...
    if($row =( mysqli_fetch_array($result))) {
        ...
        {
          echo '<script>
            window.alert("Please Fill Festival Name");
            window.location.href = "insertFestivalVideo";
            </script>';
        }
       ...
       if (isset($_FILES["video"])) {
        ...
        if (isset($_FILES["video"]) && $_FILES["video"]["error"] == UPLOAD_ERR_OK) {
            ...
            if (in_array($mime, $allowedMimeTypes)) {        
                  ...
                  move_uploaded_file($_FILES['video']['tmp_name'], $final_dir.$_FILES['video']['name']);
                  $video_full_path = $final_dir . $_FILES['video']['name']; 
                  $qry="Insert into festival(f_id,p_id,f_name,pgdName,monthName,video,detail_info) values(' ','$id','$fName','$pgdName','$monName','$video_full_path','$detail')";
                  $res=mysqli_query($conn,$qry);
                    if($res)     
                        ...
                        echo '<script>
                        window.alert("File type is not allowed. Allowed video types:[mp4]");
                        window.location.href = "insertFestivalVideo";
                        </script>';
                      ...
                      echo '<script>
                        window.alert("File size is too large. Allowed sizes:[900MB]");
                        window.location.href = "insertFestivalVideo";
                        </script>';
                    } 
                  mysqli_close($conn);
