        ...
        <div class="container-fluid">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb my-0 ms-2">
              ...
              <li class="breadcrumb-item active"><span>Update Ecclesiastes</span></li>
            </ol>
          </nav>
          <div align="right">
        ...
        <div class="row justify-content-center">
          <div class="col-md-10">
            <div class="card mb-4 mx-4">
              <div class="card-title"><h3><b>ဒေသနာတော်များပြင်ရန်</b></h3></div>
              <div class="card-body p-7">
           <form action="updateEcclesiastes" method="post" enctype="multipart/form-data">
          
                      
                  <?php
                  $e_id=$_GET['eid'];
                  $p_id=$_GET['pid']; 
                  $result = mysqli_query($conn,"SELECT * FROM ecclesiastes where p_id='$p_id' and e_id='$e_id'");
                   if($row=mysqli_fetch_array($result)){
                  ?>                         
                ...
                <img  src="assets/img/icon1/p3.png" alt="" width="30px" height="20px"></span> 
                  <!-- <input class="form-control" name="image" type="textarea" placeholder="Description"> -->
                  <!-- <input type="text" class="form-control" name="catName" id="" rows="5" value="ဖူးမြော်လေ့လာနိုင်သောနေရာများ"> -->
                ...
                <div class="input-group mb-3 mt-4" style="font-size:.5rem;">
                  ...
                  <?php                  
                  $result = mysqli_query($conn,"SELECT * FROM ecclesiastes");
...
<?php 
  ...
  else
  {
       ...
       // Allow certain file formats 
        ...
        if(($image_full_path=='upload/ecclesiastes/') ||($image_full_path1=='upload/ecclesiastes/') || ( empty($detail))){
          $result = mysqli_query($conn,"SELECT * FROM ecclesiastes where  e_id='$subCatName' and p_id='$pgdId' ");
         if($result->num_rows>0){        
          if($row=mysqli_fetch_array($result)){
              if($image_full_path=='upload/ecclesiastes/')
                  $image_full_path=$row['icon'];
              if($image_full_path1=='upload/ecclesiastes/')
           ...
           $qry="Update ecclesiastes set icon='$image_full_path', photo='$image_full_path1',detail_info='$detail' where e_id='$subCatName' and p_id='$pgdId'"; 
           $res=mysqli_query($conn,$qry);            
           if($res)     
           { 
            echo '<script>
            window.alert("Your Updated Data successfully");
            window.location.href = "viewEcclesiastes";
            </script>'; 
           }  
            else
              {
